    ...
    <div class="modal-dialog modal-xl">
                                            ...
                                            <select class="form-select" id="comunidadSelect" name="id_comunidad">
                                                ...
                                                if (!empty($data['id_parroquia'])) {
                                                    ...
                                                    while ($row = $comunidades->fetch_assoc()) {
                                                        $selected = ($row['comunidad'] == $data['comunidad']) ? 'selected' : '';
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="number" min="0" max="100" class="form-control" id="limpieza" name="limpieza" 
                                                   value="<?php echo $data['limpieza'] ?? 0; ?>" onchange="calcularAcondicionamiento()">
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="number" min="0" max="100" class="form-control" id="replanteo" name="replanteo" 
                                                   value="<?php echo $data['replanteo'] ?? 0; ?>" onchange="calcularAcondicionamiento()">
                                        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="info-grid">
                    <div class="info-item">
                        <div class="info-label"style="color: #000000">Fecha Actualización</div>
...
function calcularAvanceFisico() {
    ...
    const camposAvance = [
        'limpieza', 'replanteo', 'fundacion', 'excavacion',
    ...
    // Calcular el promedio solo si hay campos válidos
    const promedio = camposValidos > 0 ? (suma / camposValidos) : 0;
    
    // Actualizar el campo de avance físico
    document.getElementById('avance_fisico').value = promedio.toFixed(2);
}

// Agregar el evento onchange a todos los campos que afectan el avance físico
document.addEventListener('DOMContentLoaded', function() {
    const camposAvance = [
        'limpieza', 'replanteo', 'excavacion',
        'acero_vigas_riostra', 'encofrado_malla',
        'instalaciones_electricas_sanitarias', 'vaciado_losa_anclajes',
        'armado_columnas', 'vaciado_columnas',
        'armado_vigas', 'vaciado_vigas',
        'bloqueado', 'colocacion_correas', 'colocacion_techo',
        'colocacion_ventanas', 'colocacion_puertas_principales',
        'instalaciones_electricas_sanitarias_paredes', 'frisos',
        'sobrepiso', 'ceramica_bano', 'colocacion_puertas_internas',
        'equipos_accesorios_electricos', 'equipos_accesorios_sanitarios',
    ...
    // Calcular el avance físico inicial
...
function calcularAcondicionamiento() {
    ...
    document.getElementById('acondicionamiento').value = promedio.toFixed(2);
}

// Calcular el acondicionamiento inicial
document.addEventListener('DOMContentLoaded', function() {
...
function actualizarBeneficiario() {
    ...
    const camposRequeridos = ['nombre_beneficiario', 'cedula', 'status'];
    ...
    .then(data => {
        if (data.success) {
            ...
            // Cerrar el modal
            const modal = bootstrap.Modal.getInstance(document.getElementById('actualizarBeneficiarioModal'));
            if (modal) {
                modal.hide();
            }
...
document.getElementById('parroquiaSelect').addEventListener('change', function() {
